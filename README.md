...
- [remote cam 앱 요구사항](https://nextstep.camp/courses/-Kv6Po9QBOUw9ZzXzI5m/-KvWHAJiDiZM2CNHXiAM/lessons)을 파악한다.
...
ex) https://github.com/godrm/swift-remotecam
```

3. clone한 프로젝트 이동
```
cd {저장소 아이디}
ex) cd swift-remotecam
```
...
ex) git checkout -b remotecam-step1
```
...
ex) git push --set-upstream origin remotecam-step1
```
...
8. pull request
	...
	```
	ex) code-squad/swift-remotecam godrm 브랜치 기준 => godrm/swift-remotecam remotecam-step1
	```
...
10. 기본(upstream) 브랜치 전환 및 base 저장소 추가하기(최초 시작하기 단계 한번만 하면 됨)
	...
	ex) git remote add upstream https://github.com/code-squad/swift-remotecam.git
	```
